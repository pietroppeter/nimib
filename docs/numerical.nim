...
nbInit
nb.useLatex
let filename_default_style = nb.filename.replace(".html", "_default_style.html")
when not defined(numericalDefaultStyle):
  ...
  let otherStyle = fmt"; _you are looking at default style, for custom style [click here]({(nb.filename.AbsoluteFile).relPath})_"
nbText: fmt"""
